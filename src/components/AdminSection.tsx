...
export function AdminSection({ 
  ...
  const PropInput = ({ 
    ...
    return (
      <Input 
        ...
        disabled={disabled}
        className={cn("h-8 md:h-10 px-2 text-xs sm:text-sm bg-transparent hover:bg-stone-50 dark:hover:bg-white/[0.06] focus:bg-stone-50 dark:focus:bg-white/[0.08] transition-all shadow-none focus:shadow-sm font-fira-sans font-medium", disabled ? "border-transparent text-slate-800 dark:text-slate-200 opacity-100 placeholder:text-transparent" : "border-transparent hover:border-slate-200 dark:hover:border-white/[0.12] focus:border-orange-400 dark:focus:border-orange-500", className)}
      />
    );
  };
  ...
  const renderPropTable = (title: string, propItems: { plan: LessonPlan; prop: PropItem }[]) => {
    ...
    return (
      <div className="w-full bg-white dark:bg-[hsl(var(--bar-theme-border))] border rounded-2xl border-slate-300 dark:border-white/[0.08] overflow-hidden mb-8 transition-colors shadow-sm dark:shadow-[0_10px_30px_rgba(0,0,0,0.35)]">
        <div className="py-3 px-4 flex justify-between items-center border-b border-slate-300 dark:border-white/[0.08] bg-stone-50 dark:bg-black/20">
        ...
        <div className="w-full overflow-x-auto touch-pan-x touch-pan-y overscroll-x-contain">
          <table className="w-full text-sm text-left border-collapse table-fixed">
             <thead className="text-stone-500 dark:text-slate-200 text-[10px] font-fira-code font-black uppercase tracking-[0.2em] sticky top-0 bg-white/90 dark:bg-black/20 backdrop-blur-xl z-10 border-b border-slate-300 dark:border-white/[0.08]">
              <tr>
                <th className="w-[12%] px-4 py-3 border-b border-r border-slate-300 dark:border-white/[0.06]">{t('CATEGORY')}</th>
                <th className="w-[14%] px-4 py-3 border-b border-r border-slate-300 dark:border-white/[0.06]">{t('SUBJECT')}</th>
                <th className="w-[12%] px-4 py-3 border-b border-r border-slate-300 dark:border-white/[0.06]">{t('ASSIGNED_PERSONNEL')}</th>
                <th className="w-[16%] px-4 py-3 border-b border-slate-300 dark:border-white/[0.06] min-w-[100px] border-r border-slate-300 dark:border-white/[0.06]">{t('PROP_NAME')}</th>
                <th className="w-[8%] px-4 py-3 border-b border-slate-300 dark:border-white/[0.06] whitespace-nowrap border-r border-slate-300 dark:border-white/[0.06]">Qty</th>
                <th className="w-[8%] px-4 py-3 border-b border-slate-300 dark:border-white/[0.06] whitespace-nowrap border-r border-slate-300 dark:border-white/[0.06]">Unit</th>
                <th className="w-[16%] px-4 py-3 border-b border-slate-300 dark:border-white/[0.06] min-w-[100px] border-r border-slate-300 dark:border-white/[0.06]">{t('OP_REMARKS')}</th>
                <th className="w-[7%] px-4 py-3 text-center border-b border-slate-300 dark:border-white/[0.06] whitespace-nowrap border-r border-slate-300 dark:border-white/[0.06]">{t('PACKED')}</th>
                <th className="w-[7%] px-4 py-3 text-center border-b border-slate-300 dark:border-white/[0.06] whitespace-nowrap border-r border-slate-300 dark:border-white/[0.06]">{t('CHECKED')}</th>
              </tr>
            </thead>
            <tbody>
                  ...
                  planItems.map((item, iIndex) => {
                    ...
                    return (
                      <tr key={`${item.plan.id}-${item.prop.id}`} className={cn(
                        "group border-b border-slate-300 dark:border-white/[0.06] hover:bg-stone-50 dark:hover:bg-stone-50/[0.04] transition-colors duration-200",
                        item.prop.isFromClub && item.prop.isToPurchase ? "bg-emerald-50/30 dark:bg-emerald-900/12" : "bg-white dark:bg-transparent"
                      )}>
                        {isFirstInGroup && (
                          <td className="px-4 py-3 font-fira-code font-black text-xs sm:text-sm text-slate-800 dark:text-slate-400 align-top border-r border-slate-300 dark:border-white/[0.06]" rowSpan={items.length}>
                            {categoryName}
                          </td>
                        )}
                        {isFirstInPlan && (
                          <td className="px-4 py-3 font-bold text-xs sm:text-sm text-slate-700 dark:text-slate-300 align-top border-r border-slate-300 dark:border-white/[0.06]" rowSpan={planItems.length}>
                            {item.plan.activityName || '-'}
                          </td>
                        )}
                        {isFirstInPlan && (
                          <td className="px-4 py-3 text-xs sm:text-sm text-slate-600 dark:text-slate-400 align-top border-r border-slate-300 dark:border-white/[0.06]" rowSpan={planItems.length}>
                            {item.plan.members || '-'}
                          </td>
                        )}
                        <td className="px-2 py-2 align-middle border-r border-slate-300 dark:border-white/[0.06]">
                          <PropInput 
                        ...
                        <td className="px-4 py-3 text-center align-middle border-r border-slate-300 dark:border-white/[0.06]">
                          <div className="flex justify-center items-center h-full">
                            <Checkbox 
                              ...
                              className="h-5 w-5 border-2 dark:border-white/30 data-[state=checked]:bg-emerald-500 data-[state=checked]:border-emerald-500"
                            />
                          </div>
                        </td>
                        <td className="px-4 py-3 text-center align-middle border-r border-slate-300 dark:border-white/[0.06]">
  ...
  const renderCombinedTable = () => {
    ...
    return (
      <div className="w-full bg-white dark:bg-[hsl(var(--bar-theme-border))] border rounded-2xl border-slate-300 dark:border-white/[0.08] overflow-hidden mb-8 transition-colors shadow-sm dark:shadow-[0_10px_30px_rgba(0,0,0,0.35)]">
        <div className="py-3 px-4 flex justify-between items-center border-b border-slate-300 dark:border-white/[0.08] bg-stone-50 dark:bg-black/20">
          <h2 className="text-center font-fira-code font-black text-stone-900 dark:text-slate-100 tracking-[0.1em] uppercase text-sm">{t('PROPS_LIST')}</h2></div>
        
        <div className="w-full overflow-x-auto touch-pan-x touch-pan-y overscroll-x-contain">
          <table className="w-full text-sm text-left border-collapse table-fixed">
            <thead className="text-stone-500 dark:text-slate-200 text-[10px] font-fira-code font-black uppercase tracking-[0.2em] sticky top-0 z-10 bg-white/90 dark:bg-black/20 backdrop-blur-xl border-b border-slate-300 dark:border-white/[0.08]">
              <tr>
                <th className="w-[16%] px-4 py-3 border-b border-r border-slate-300 dark:border-white/[0.06]">{t('PROP_USAGE')}</th>
                <th className="w-[16%] px-4 py-3 border-b border-slate-300 dark:border-white/[0.06] min-w-[100px] border-r border-slate-300 dark:border-white/[0.06]">{t('PROP_NAME')}</th>
                <th className="w-[16%] px-4 py-3 border-b border-r border-slate-300 dark:border-white/[0.06]">{t('ASSIGNED_PERSONNEL')}</th>
                <th className="w-[28%] px-4 py-3 border-b border-r border-slate-300 dark:border-white/[0.06]">{t('MATERIALS')}</th>
                <th className="w-[12%] px-4 py-3 text-center border-b border-slate-300 dark:border-white/[0.06] whitespace-nowrap border-r border-slate-300 dark:border-white/[0.06]">{t('PACKED')}</th>
                <th className="w-[12%] px-4 py-3 text-center border-b border-slate-300 dark:border-white/[0.06] whitespace-nowrap border-r border-slate-300 dark:border-white/[0.06]">{t('CHECKED')}</th>
                {!isLocked && <th className="w-[10%] px-4 py-3 text-center border-b border-r border-slate-300 dark:border-white/[0.06]">操作</th>}
              </tr>
            </thead>
            
            {/* 1. 活動組 */}
            <tbody>
              <tr><td colSpan={isLocked ? 6 : 7} className="px-4 py-4 font-fira-code font-black text-orange-600 dark:text-amber-500 text-xs sm:text-sm uppercase tracking-widest bg-transparent border-b border-slate-300 dark:border-white/[0.06]">活動組 - 教案道具確認</td></tr>
              {Object.keys(activityGroups).length === 0 ? (
                <tr><td colSpan={isLocked ? 6 : 7} className="text-center py-6 text-slate-400 dark:text-slate-500 font-bold border-b border-slate-300 dark:border-white/[0.06]">目前沒有活動組資料</td></tr>
                ...
                catePlans.map((plan, pIndex) => (
                  <tr key={`act-${plan.id}`} className={cn(
                    ...
                    {pIndex === 0 && (
                      <td className="px-4 py-3 font-fira-code font-black text-xs sm:text-sm text-slate-800 dark:text-slate-400 align-top border-r border-slate-300 dark:border-white/[0.06]" rowSpan={catePlans.length}>
                        {categoryName}
                      </td>
                    )}
                    <td className="px-4 py-3 font-bold text-xs sm:text-sm text-slate-700 dark:text-slate-300 align-top border-r border-slate-300 dark:border-white/[0.06]">
                      {plan.activityName || '-'}
                    </td>
                    <td className="px-4 py-3 text-xs sm:text-sm text-slate-600 dark:text-slate-400 align-top border-r border-slate-300 dark:border-white/[0.06]">
                      {plan.members || '-'}
                    </td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300 text-xs sm:text-sm border-r border-slate-300 dark:border-white/[0.06]">
                      {plan.props.length > 0 ? (
                    ...
                    <td className="px-4 py-3 text-center align-middle border-r border-slate-300 dark:border-white/[0.06]">
                      <div className="flex justify-center items-center h-full">
                        <Checkbox checked={plan.isPropsPacked || false} disabled={isLocked} onCheckedChange={(c) => onUpdatePlan(plan.id, { isPropsPacked: c === true })} className="h-5 w-5 border-2 dark:border-white/30 data-[state=checked]:bg-emerald-500 data-[state=checked]:border-emerald-500" />
                      </div>
                    </td>
                    <td className="px-4 py-3 text-center align-middle border-r border-slate-300 dark:border-white/[0.06]">
                      <div className="flex justify-center flex-col items-center h-full">
                        <Checkbox checked={plan.isPreDepartureChecked || false} disabled={isLocked} onCheckedChange={(c) => onUpdatePlan(plan.id, { isPreDepartureChecked: c === true })} className="h-5 w-5 border-2 dark:border-white/30 data-[state=checked]:bg-emerald-500 data-[state=checked]:border-emerald-500" />
                      </div>
                    </td>
                    {!isLocked && <td className="px-4 py-3 text-center align-middle border-r border-slate-300 dark:border-white/[0.06]"></td>}
                  </tr>
                ))
              ))}
            </tbody>
            ...
            {/* 2. 教學組 */}
            <tbody>
              <tr><td colSpan={isLocked ? 6 : 7} className="px-4 py-4 font-fira-code font-black text-blue-600 dark:text-blue-400 text-xs sm:text-sm uppercase tracking-widest bg-transparent border-b border-slate-300 dark:border-white/[0.06]">教學組 - 教案道具確認</td></tr>
              {Object.keys(teachingGroups).length === 0 ? (
                <tr><td colSpan={isLocked ? 6 : 7} className="text-center py-6 text-slate-400 dark:text-slate-500 font-bold border-b border-slate-300 dark:border-white/[0.06]">目前沒有教學組資料</td></tr>
            ...
            {/* 3. 營期物品 */}
            <tbody>
              <tr><td colSpan={isLocked ? 6 : 7} className="px-4 py-4 font-fira-code font-black text-emerald-600 dark:text-emerald-400 text-xs sm:text-sm uppercase tracking-widest bg-transparent border-b border-slate-300 dark:border-white/[0.06]">營期其他物品確認</td></tr>
              {Object.keys(usageGroups).length === 0 ? (
                <tr><td colSpan={isLocked ? 6 : 7} className="text-center py-4 text-slate-400 dark:text-slate-600 font-bold border-b border-slate-300 dark:border-white/[0.06]">目前沒有營期物品資料</td></tr>
                ...
                items.map((item, pIndex) => (
                  <tr key={`cmp-${item.id}`} className={cn(
                    ...
                    <td className="px-4 py-3 align-middle border-r border-slate-300 dark:border-white/[0.06]">
                       <PropInput 
                          value={item.name}
                    ...
                    <td className="px-4 py-3 text-xs sm:text-sm text-slate-400 dark:text-slate-600 align-middle border-r border-slate-300 dark:border-white/[0.06]">
                      -
                    </td>
                    <td className="px-4 py-3 text-xs sm:text-sm text-slate-400 dark:text-slate-600 align-middle border-r border-slate-300 dark:border-white/[0.06]">
                      -
  ...
  return (
    <div className="h-full flex flex-col bg-stone-50 dark:bg-[hsl(var(--bar-theme))] animate-in fade-in duration-500 relative transition-colors font-fira-sans overflow-hidden">
      <main className="flex-1 w-full h-full overflow-y-auto min-h-0 relative">
        <div className="min-h-full w-full pt-28 sm:pt-24 px-4 sm:px-6 md:px-8 lg:px-10 pb-8 md:pb-12">
          <Tabs value={activeMainTab} onValueChange={handleMainTabChange} className="w-full flex flex-col items-stretch space-y-6">
            ...
            <ActionBar title="Admin Actions" className="md:justify-end">
              ...
              <Button variant="ghost" size="sm" onClick={handlePrint} className="h-9 px-3 rounded-lg font-bold text-xs bg-transparent text-slate-900 dark:text-white hover:opacity-100 opacity-90 transition-opacity">
                匯出 / PRINT
              </Button>
              <Button variant="ghost" size="icon" onClick={onUndoTable} disabled={!canUndoTable || isLocked} className="h-9 w-9 rounded-lg bg-transparent text-slate-900 dark:text-white hover:opacity-100 opacity-90 transition-opacity">
                <Undo2 className="h-4 w-4" />
              </Button>
              <Button variant="ghost" size="icon" onClick={onRedoTable} disabled={!canRedoTable || isLocked} className="h-9 w-9 rounded-lg bg-transparent text-slate-900 dark:text-white hover:opacity-100 opacity-90 transition-opacity">
                <Redo2 className="h-4 w-4" />
              </Button>
              <Button variant="ghost" size="icon" onClick={handleZoomOut} disabled={zoom <= 0.3 || activeMainTab === 'timer'} className="h-9 w-9 rounded-lg bg-transparent text-slate-900 dark:text-white hover:opacity-100 opacity-90 transition-opacity">
                <ZoomOut className="h-4 w-4" />
              </Button>
              <Button variant="ghost" size="icon" onClick={handleFitAll} disabled={activeMainTab === 'timer'} className="h-9 w-9 rounded-lg bg-transparent text-slate-900 dark:text-white hover:opacity-100 opacity-90 transition-opacity">
                <Maximize className="h-4 w-4" />
              </Button>
              <Button variant="ghost" size="icon" onClick={handleZoomIn} disabled={zoom >= 2 || activeMainTab === 'timer'} className="h-9 w-9 rounded-lg bg-transparent text-slate-900 dark:text-white hover:opacity-100 opacity-90 transition-opacity">
        ...
        <div className="w-full flex-1 relative">
            ...
            <TabsContent value="tables" className="m-0 data-[state=active]:flex flex-col space-y-6 md:space-y-8 pb-32">
              <div className="bg-orange-50/20 dark:bg-slate-900/50 rounded-2xl border border-orange-100 dark:border-white/[0.06] p-4 shrink-0 flex flex-wrap md:flex-nowrap items-center justify-between gap-4 mt-2">
                <div className="flex items-center gap-4">
                  ...
                  <Select value={selectedDay} onValueChange={setSelectedDay}>
                    <SelectTrigger className="w-32 h-8 rounded-lg font-fira-code font-black text-[10px] border-orange-200 dark:border-white/[0.12] bg-white dark:bg-slate-800 dark:text-slate-200 shadow-sm focus:ring-0 transition-colors">
                      <SelectValue placeholder="選擇" />
                    </SelectTrigger>
                    <SelectContent className="rounded-xl border-orange-100 dark:border-white/[0.12] shadow-2xl dark:bg-slate-800 dark:text-slate-200 font-fira-code">
                      {dayOptions.map(day => (
                ...
                >
                  {filteredTables.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-24 text-center space-y-6">
                      <div className="w-16 h-16 rounded-3xl bg-white dark:bg-slate-800/50 flex items-center justify-center text-orange-200 dark:text-slate-600 shadow-sm border border-orange-50 dark:border-white/[0.12]/50">
