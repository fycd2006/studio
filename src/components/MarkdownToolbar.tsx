...
export function MarkdownToolbar({ className }: { className?: string }) {
  const toolbarIconButtonClass = "h-8 w-8 sm:h-9 sm:w-9 rounded-lg text-stone-600 dark:text-foreground/90 hover:bg-stone-200/70 dark:hover:bg-white/10 active:bg-stone-300/70 dark:active:bg-white/15 transition-colors shrink-0";
  ...
  return (
    <div 
      ...
      className={cn(
        "flex flex-nowrap items-center gap-0.5 sm:gap-1 p-0 sm:p-0.5 bg-transparent rounded-lg transition-all w-full select-none overflow-x-auto no-scrollbar scroll-smooth",
        className
      )}
    >
      <TooltipProvider>
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="ghost" size="sm" className="h-8 gap-1 text-[11px] font-bold text-stone-700 dark:text-foreground/90 rounded-lg hover:bg-stone-200/50 dark:hover:bg-white/10 shrink-0">
              <FontIcon className="h-3.5 w-3.5" />
              <span className="hidden sm:inline">格式</span>
              <ChevronDown className="h-3 w-3 opacity-60" />
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-40 p-1.5 rounded-xl border border-stone-200/80 dark:border-white/10 z-[99]" align="start">
        ...
        <Popover>
          ...
          <PopoverContent className="w-40 p-1.5 rounded-xl border border-stone-200/80 dark:border-white/10 z-[99]" align="start">
            <div className="flex flex-col gap-1 max-h-56 overflow-y-auto no-scrollbar">
              ...
              <div className="p-1 border-t border-stone-200/80 dark:border-white/10 mt-1.5 pt-1.5 flex items-center gap-1.5 shrink-0">
                <Input
                  type="number"
                  placeholder="px"
                  min="8"
        ...
        <div className="w-px h-4 bg-stone-200 dark:bg-white/10 mx-1 shrink-0" />
        ...
        <Tooltip>
          <Popover>
            ...
            <PopoverContent className="w-48 p-3 rounded-2xl shadow-xl z-[99] border border-stone-200/80 dark:border-white/10" side="bottom" align="start">
              <div className="space-y-3">
                <div>
                  <span className="text-[10px] font-bold text-fg-muted font-mono uppercase tracking-widest block mb-1.5">文字顏色 / Text Color</span>
                  <div className="grid grid-cols-5 gap-1.5">
                    ...
                    <button type="button" className="w-6 h-6 rounded-full cursor-pointer hover:scale-105 border border-stone-200/80 dark:border-white/10 shadow-sm flex items-center justify-center bg-gradient-to-tr from-rose-400 via-amber-400 to-blue-500" onClick={() => textColorInputRef.current?.click()} title="自訂顏色">
                      <span className="text-[12px] font-bold text-white shadow-sm">+</span>
                    </button>
                  </div>
                </div>
                <div>
                  <span className="text-[10px] font-bold text-fg-muted font-mono uppercase tracking-widest block mb-1.5">螢光筆高亮 / Highlight</span>
                  <div className="grid grid-cols-5 gap-1.5">
                    {[
                      { value: 'transparent', className: 'bg-transparent border border-stone-200/80 dark:border-white/10' },
                    ...
                    <button type="button" className="w-6 h-6 rounded-full cursor-pointer hover:scale-105 border border-stone-200/80 dark:border-white/10 shadow-sm flex items-center justify-center bg-gradient-to-tr from-rose-400 via-amber-400 to-blue-500" onClick={() => highlightColorInputRef.current?.click()} title="自訂高亮">
                      <span className="text-[12px] font-bold text-white shadow-sm">+</span>
                    </button>
                  </div>
                </div>
        ...
        <Tooltip>
          <Popover>
            ...
            <PopoverContent className="w-40 p-2 rounded-xl z-[99] border border-stone-200/80 dark:border-white/10" side="bottom" align="start">
              <span className="text-[10px] font-bold text-fg-muted font-mono uppercase tracking-widest block mb-2">插入表格 / Table</span>
        ...
        <Tooltip>
          <Popover open={isLinkOpen} onOpenChange={setIsLinkOpen}>
            ...
            <PopoverContent className="w-64 p-3 rounded-2xl z-[99] border border-stone-200/80 dark:border-white/10 shadow-xl" side="bottom" align="start">
              <form onSubmit={insertLink} className="space-y-2">
                <span className="text-[10px] font-bold text-fg-muted font-mono uppercase tracking-widest block mb-1">插入連結 / Insert Link</span>
                <Input
                  type="url"
                ...
                <div className="flex justify-end gap-1.5 pt-1">
                  ...
                  <Button type="submit" size="sm" className="h-7 px-3.5 rounded-lg text-xs bg-orange-600 dark:bg-amber-400 text-white dark:text-[#0B1012] border-none">插入</Button>
                </div>
              </form>
            </PopoverContent>
          </Popover>
          ...
          <Popover open={isImageOpen} onOpenChange={setIsImageOpen}>
            ...
            <PopoverContent className="w-64 p-3 rounded-2xl z-[99] border border-stone-200/80 dark:border-white/10 shadow-xl" side="bottom" align="start">
              <form onSubmit={insertImageUrl} className="space-y-2">
                <span className="text-[10px] font-bold text-fg-muted font-mono uppercase tracking-widest block mb-1">插入圖片 URL / Image</span>
                <Input
                  type="url"
