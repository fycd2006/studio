...
# 1. Inject handlers before return statement of component
helper_code = """
...
if "const handleUpdatePlanGroup" not in content:
    content = content.replace("  return (", helper_code, 1)

# 2. Replace grid view badge with helper
grid_badge_pattern = re.compile(r'<Badge className=\{cn\("px-2\.5 py-1 text-\[10px\] font-extrabold[^>]+>\s*\{\(\(\) => \{[^}]+\}\)\(\)\}\s*<\/Badge>', re.MULTILINE)
content = grid_badge_pattern.sub(r'{renderGroupBadge(plan)}', content)
grid_badge_pattern_fallback = re.compile(r'<Badge className=\{cn\("px-2 py-0\.5[^>]+>\s*\{\(\(\) => \{[^}]+\}\)\(\)\}\s*<\/Badge>', re.MULTILINE)
content = grid_badge_pattern_fallback.sub(r'{renderGroupBadge(plan)}', content)


# 3. Replace list view badge with helper
list_badge_pattern = re.compile(r'<td className="px-6 py-4 border-y border-transparent[^>]+>\s*<Badge[^>]+>\s*\{\(\(\) => \{[^}]+\}\)\(\)\}\s*<\/Badge>\s*<\/td>', re.MULTILINE)
content = list_badge_pattern.sub(r'<td className="px-6 py-4 border-y border-transparent group-hover:border-orange-100 dark:group-hover:border-slate-600 hidden sm:table-cell">{renderGroupBadge(plan)}</td>', content)

# 4. Replace entire Board View map logic
# Find start of board view
board_view_start = content.find(') : viewType === "board" ? (')
board_view_end = content.find(') : (\n  <div className="bg-white dark:bg-slate-800 rounded-xl')
if board_view_start != -1 and board_view_end != -1:
    new_board_view = """) : viewType === "board" ? (
 ...
 <motion.div 
   ...
   onDragStart={(e: any) => { 
     ...
     // optional ghost image customization can go here
   }}
 ...
 """
    content = content[:board_view_start] + new_board_view + content[board_view_end:]

with open('src/app/plans/page.tsx', 'w', encoding='utf-8') as f:
    f.write(content)

print("Update board success")
