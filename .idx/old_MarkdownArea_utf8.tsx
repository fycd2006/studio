      ...
      if (items[i].type.indexOf("image") !== -1) {
        ...
        if (file) {
          ...
          reader.onload = (event) => {
            ...
            const imgHtml = `<img src="${dataUri}" style="width: 50%; height: auto; display: inline-block; margin: 10px; border-radius: 12px; cursor: pointer; transition: all 0.3s; vertical-align: top;" />`;
  ...
  return (
    <div className="space-y-2">
      ...
      <div className={cn(
        ...
        {selectedImage && (
          <div 
            className="fixed z-[9999] flex flex-col gap-2 p-2 bg-slate-900/95 backdrop-blur-md rounded-2xl shadow-2xl border border-white/20 animate-in fade-in zoom-in-95 duration-200"
            style={{ 
            ...
            <div className="flex items-center gap-1.5">
              <TooltipProvider>
                ...
                <div className="flex items-center gap-1 px-1 border-r border-white/10">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-white hover:bg-white/20" onClick={() => toggleStyle('aspectRatio', '16/9')}><RectangleHorizontal className="h-4 w-4" /></Button>
                    </TooltipTrigger>
          ...
          <TooltipProvider>
            ...
            <Tooltip>
              <Popover>
                ...
                <PopoverContent className="w-40 p-1 rounded-xl border-none shadow-2xl z-[60]" side="bottom" align="start">
            ...
            <Tooltip>
              <Popover>
                ...
                <PopoverContent className="w-48 p-2 rounded-2xl border-none shadow-2xl z-[60]" side="bottom" align="start">
                  <div className="grid grid-cols-4 gap-2">
                    {['#0f172a', '#64748b', '#3b82f6', '#06b6d4', '#ef4444', '#22c55e', '#f97316', '#a855f7'].map((color) => (
                      <div key={color} className="w-7 h-7 rounded-full cursor-pointer border border-slate-100 hover:border-primary shadow-sm transition-all" style={{ backgroundColor: color }} onMouseDown={(e) => { e.preventDefault(); execCommand("foreColor", color); }} />
                    ))}
                  </div>
                </PopoverContent>
              </Popover>
